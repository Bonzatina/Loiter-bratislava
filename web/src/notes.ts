...
const SUBJECT_PREFIX = process.env.NOTES_SUBJECT_PREFIX ?? 'Loiter: Bratislava'
...
function formatBody(p: NotePayload): string {
  const { page, values } = p
  const url = `${SITE_URL || ''}${p.lang === 'en' ? '/en' : ''}/${encodeURIComponent(page.slug)}`
  const sentAt = new Intl.DateTimeFormat('sv-SE', {
    timeZone: 'Europe/Bratislava', dateStyle: 'short', timeStyle: 'short',
  }).format(new Date())
  const rows: [string, string][] = [
    ...
    ['Городская часть', `${page.district ?? '—'}${page.quarter ? ` · квартал: ${page.quarter}` : ''}`],
    ['Координаты',  page.coords ? `${page.coords[0]}, ${page.coords[1]}` : '—'],
    ['Дата визита', values.visited || '—'],
    ['Отправлено',  `${sentAt} (Europe/Bratislava)`],
    ['Язык формы',  p.lang],
...
export async function sendNote(p: NotePayload): Promise<void> {
  const subject = `[${SUBJECT_PREFIX}] Заметка · ${p.page.slug}`
